...
boost::shared_ptr<Model> RobogenUtils::createModel(
	...
	if (bodyPart.type().compare(CORE_COMPONENT) == 0) {
		...
		for (int j = 0; j < bodyPart.evolvableparam_size(); ++j) {
			...
			std::cout << "L: " << evolvableParameter.paramname() << ", angleA: " << evolvableParameter.paramvalue() <<  std::endl;
		...
		model.reset(
				new ParametricBrickModel(odeWorld, odeSpace, length, alpha,
						beta));
		...
		model.reset(new PassiveWheelModel(odeWorld, odeSpace));

	} else if (bodyPart.type().compare(ACTIVE_WHEEL) == 0) {

		model.reset(new ActiveWheelModel(odeWorld, odeSpace));

	} else if (bodyPart.type().compare(ACTIVE_WHEG) == 0) {

		model.reset(new ActiveWhegModel(odeWorld, odeSpace));
