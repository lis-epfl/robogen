...
bool PassiveWheelRenderModel::initRenderModel() {
	...
	bool meshLoadingA = this->partA_->loadMesh("models/PassiveRotation_Frame.stl");

	if (!meshLoadingA) {
		std::cerr << "[PassiveWheelRenderModel] Error loading model"
				<< std::endl;
		return false;
	}

	bool meshLoadingB = this->partB_->loadMesh("models/PassiveRotation_Wheel.stl");
...
void PassiveWheelRenderModel::showDebugView() {
	...
	osg::ref_ptr<osg::Geode> wheel = this->getCylinder(
			fromOde(PassiveWheelModel::WHEEL_BASE_RADIUS),
