...
int main(int argc, char *argv[]) {
	...
	EM_ASM(
			FS.mkdir('/virtualFS');
					FS.mount(NODEFS, {root: '.'}, '/virtualFS');
					);
					std::cout << "FileSystem mounted" << std::endl;
#endif
					std::ofstream myfile;
					myfile.open ("toto.titi");
					myfile << "gogogogogogog";
					try {
						...
						if (argc >= 4 && !boost::starts_with(argv[3], "--")) {
							...
							if (ss.fail()) {
								std::cerr
										<< "Specified desired starting position \""
										<< argv[3]
						...
						for (; currentArg < argc; currentArg++) {
							if (std::string("--help").compare(argv[currentArg])
									== 0) {
								printUsage(argv);
								return EXIT_SUCCESS;
							} else if (std::string("--record").compare(
									argv[currentArg]) == 0) {
								if (argc < (currentArg + 3)) {
									std::cerr
											<< "In order to record frames, must provide frame "
								...
								ss >> recordFrequency;
								if (ss.fail()) {
									std::cout << "Specified record frequency \""
											<< argv[currentArg]
											<< "\" is not an integer. Aborting..."
											<< std::endl;
									return EXIT_FAILURE;
							...
							} else if (std::string("--output").compare(
									argv[currentArg]) == 0) {
								if (argc < (currentArg + 2)) {
									std::cerr
											<< "In order to write output files, must provide "
											<< "directory." << std::endl;
									return EXIT_FAILURE;
							...
							} else if (std::string("--no-visualization").compare(
									argv[currentArg]) == 0) {
								visualize = false;
							} else if (std::string("--pause").compare(
									argv[currentArg]) == 0) {
								startPaused = true;
							} else if (std::string("--speed").compare(
									argv[currentArg]) == 0) {
								if (argc < (currentArg + 2)) {
									std::cerr
											<< "Must specify a speed factor with option --speed."
											<< std::endl;
									return EXIT_FAILURE;
								}
								currentArg++;
								std::stringstream ss(argv[currentArg]);
								ss >> speed;
							} else if (std::string("--debug").compare(
									argv[currentArg]) == 0) {
								debug = true;
							} else if (std::string("--seed").compare(
									argv[currentArg]) == 0) {
								currentArg++;
								std::stringstream ss(argv[currentArg]);
								ss >> seed;
							} else if (std::string("--webgl").compare(
									argv[currentArg]) == 0) {
								writeWebGL = true;
							} else if (std::string("--overwrite").compare(
									argv[currentArg]) == 0) {
								overwrite = true;
							}
						...
						if (recording && !visualize) {
							std::cerr
									<< "Cannot record without visualization enabled!"
									<< std::endl;
							return EXIT_FAILURE;
						}
						...
						if (startPaused && !visualize) {
							std::cerr
									<< "Cannot start paused without visualization enabled."
									<< std::endl;
							return EXIT_FAILURE;
						}
						...
						if (writeWebGL && (!writeLog)) {
							std::cerr
									<< "Cannot write json file for WebGL visualizer without "
						...
						if (overwrite && (!writeLog)) {
							std::cerr
									<< "No output directory was specified, so there is "
						...
						if (boost::filesystem::path(argv[1]).extension().string().compare(
							...
							robotFile.read((char*) &packetBuffer[0],
									packetSize);
							...
							if (!robot.init(argv[1])) {
								std::cerr
										<< "Failed interpreting robot text file!"
										<< std::endl;
								return EXIT_FAILURE;
							}
							...
							json2pb(robotMessage, (char*) &packetBuffer[0],
									packetSize);

						} else {
							std::cerr
									<< "File extension of provided robot file could not be "
						...
						unsigned int simulationResult = runSimulations(scenario,
								configuration, robotMessage, viewer, rng, true,
								log);
